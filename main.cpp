...
int main(int argc, char *argv[]) {
  ...
  long int magazineCapacity = strtol(argv[1], &pEnd, 10);
  long int producers = strtol(argv[2], &pEnd, 10);
  long int consumers = strtol(argv[3], &pEnd, 10);
  long int prodMin = strtol(argv[4], &pEnd, 10);
  long int prodMax = strtol(argv[5], &pEnd, 10);
  long int consMin = strtol(argv[6], &pEnd, 10);
  long int consMax = strtol(argv[7], &pEnd, 10);

  monitor.setCapacity(magazineCapacity);

  int resultCode;
  pthread_t threads[producers + consumers];
  struct producer producerThreads[producers];
  struct consumer consumerThreads[consumers];
  int i;

  clearLogs();

  for (i = 0; i < producers; i++) {
    struct producer prod = {prodMin, prodMax, i};
    pthread_mutex_init(&prod.mutex, nullptr);
    producerThreads[i] = prod;
    resultCode =
        pthread_create(&threads[i], nullptr, produce, &producerThreads[i]);
    assert(!resultCode);
  }

  for (i = 0; i < consumers; i++) {
    struct consumer cons = {consMin, consMax, i};
    pthread_mutex_init(&cons.mutex, nullptr);
    consumerThreads[i] = cons;
    resultCode = pthread_create(&threads[i + producers], nullptr, consume,
                                &consumerThreads[i]);
    assert(!resultCode);
  }
